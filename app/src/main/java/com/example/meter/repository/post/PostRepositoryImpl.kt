...
class PostRepositoryImpl @Inject constructor(private val apiService: ApiService): PostRepository {
    override suspend fun searchPosts(query: String): Response<List<SellCarPost>> = apiService.searchPosts(query)
    override suspend fun getLatestPosts(): Response<List<SellCarPost>> =apiService.getLatestPosts()
}